...
module.exports = {
  data: new SlashCommandBuilder().setName('criarcupom').setDescription('Criar cupom de desconto').setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
  async execute(i) {
    ...
    m.addComponents(
      ...
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('dias').setLabel('Validade em dias').setStyle(TextInputStyle.Short).setValue('30').setRequired(true))
    );
    await i.showModal(m);
  }
};
